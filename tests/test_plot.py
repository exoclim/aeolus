...
def example_cube_2d():
    xc = iris.coords.DimCoord(
        [-1, 2, 3], units="degrees", standard_name="longitude"
    )
    yc = iris.coords.DimCoord(
        [10, 30, 50, 70], units="degrees", standard_name="latitude"
    )
...
def test_cube_minmeanmax_str(example_cube_2d):
    assert (
        plot.cube_minmeanmax_str(example_cube_2d) == "min=0 | mean=4 | max=11"
    )
...
def test_unit_format():
    ...
    assert (
        plot.unit_format(1.234e5, unit="m s^{-1}")
        == "$1.2\\times10^{5}$ $m$ $s^{-1}$"
    )
